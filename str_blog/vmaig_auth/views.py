...
class UserControl(View):
    ...
    def changetx(self,request):
        ...
        try:
            ...
            request.user.img = "http://7xpgaf.com1.z0.glb.clouddn.com/"+filename + "?v" + time.strftime('%Y%m%d%H%M%S')
            #request.user.img = "http://7xpgaf.com1.z0.glb.clouddn.com/"+filename 
            request.user.save()
            #验证上传是否错误
