...
class BaseMixin(object):
    ...
    def get_context_data(self,*args,**kwargs):
        ...
        try:
            #热门套图
            context['hot_article_list'] = Article.objects.filter(category=1).order_by("-view_times")[0:5]
            context['hot_video_list'] = Article.objects.filter(category=2).order_by("-view_times")[0:5]
            #context['hot_article_list'] = Article.objects.order_by("-view_times")[0:10]
            #导航条
            context['nav_list'] =  Nav.objects.filter(status=0)
            #最新评论
            context['latest_comment_list'] = Comment.objects.order_by("-create_time")[0:5]
...
class IndexView(PaginationMixin,BaseMixin,ListView):
...
class AllView(BaseMixin,ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for article in article_list:
            html +=  template.loader.get_template('blog/include/home_post.html').render(template.Context({'post':article}))

        mydict = {"html":html,"isend":isend}
        return HttpResponse(json.dumps(mydict),content_type="application/json")


class SearchView(PaginationMixin,BaseMixin,ListView):
    ...
    def get_context_data(self,**kwargs):
        ...
        kwargs['count'] = Article.objects.only('title','summary','tags')\
                .filter(Q(title__icontains=kwargs['s'])|Q(summary__icontains=kwargs['s'])|Q(tags__icontains=kwargs['s'])\
                ,status=0).count()
    ...
    def post(self, request, *args, **kwargs):
        #request.POST is a dict, get根据key值获取val, 第二个参数是获取不到时候的default
        val = self.request.POST.get("val","")
        sort = self.request.POST.get("sort","time")
        start = self.request.POST.get("start",0)
        end = self.request.POST.get("end",PAGE_NUM)
        ...
        try:
            article_list = Article.objects.only('title','summary','tags')\
                .filter(Q(title__icontains=val)|Q(summary__icontains=val)|Q(tags__icontains=val)\
                ,status=0).order_by(sort)[start:end+1]
        except Category.DoesNotExist:
            logger.error(u'[TagView]此关键字不存在:[%s]' % val)
            raise PermissionDenied
        ...
        html = ""
        for article in article_list:
            html +=  template.loader.get_template('blog/include/search_post.html').render(template.Context({'post':article}))

        mydict = {"html":html,"isend":isend}
        return HttpResponse(json.dumps(mydict),content_type="application/json")


class TagView(PaginationMixin,BaseMixin,ListView):
    ...
    def get_context_data(self,**kwargs):
        ...
        kwargs['PAGE_NUM'] = PAGE_NUM
        kwargs['count'] = Article.objects.only('tags').filter(tags__icontains=kwargs['tag'],status=0).count()
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            article_list = Article.objects.only('tags').filter(tags__icontains=val,status=0).order_by(sort)[start:end+1]
        except Category.DoesNotExist:
            logger.error(u'[TagView]此标签不存在:[%s]' % val)
            raise PermissionDenied
        ...
        html = ""
        for article in article_list:
            html +=  template.loader.get_template('blog/include/tag_post.html').render(template.Context({'post':article}))

        mydict = {"html":html,"isend":isend}
        return HttpResponse(json.dumps(mydict),content_type="application/json")

class CategoryView(PaginationMixin,BaseMixin,ListView):
    ...
    def get_context_data(self,**kwargs):
        ...
        kwargs['PAGE_NUM'] = PAGE_NUM
        kwargs['count'] = Category.objects.get(name=kwargs['category']).article_set.count()
        return super(CategoryView,self).get_context_data(**kwargs)
        
    #似乎category也要添加post处理函数，不然具体的分类要怎么才能传过去？1、通过url传过去，如果直接用all视图中的来处理是否可以把具体的分类参数传过去？
    def post(self, request, *args, **kwargs):
        #request.POST is a dict, get根据key值获取val, 第二个参数是获取不到时候的default
        val = self.request.POST.get("val","")
        sort = self.request.POST.get("sort","time")
        ...
        if val == "all":
            ...
            try:
                article_list = Category.objects.get(name=val).article_set.all()
                article_list = article_list[start:end+1]
            except Category.DoesNotExist:
                logger.error(u'[AllView]此分类不存在:[%s]' % val)
                raise PermissionDenied
        ...
        html = ""
        for article in article_list:
            html +=  template.loader.get_template('blog/include/category_post.html').render(template.Context({'post':article}))
...
class ColumnView(PaginationMixin,BaseMixin,ListView):
